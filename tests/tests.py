...
def setUp(self):
    ...
    '''
    self.user_jojo = User(username = 'jojo',firstname='testname1',lastname='testname2',password = 'passtest', email = 'test@test.com')
    self.pitch_one = Pitch(pitch = 'heyTest',category = 'Product',user = self.user_jojo)
    self.new_comment = Comment(comment = 'nice test',pitch = self.pitch_one,user = user_jojo)
...
def test_check_instance_variables(self):
    ...
    self.assertEquals(self.pitch_one.user,self.user_jojo)
...
def test_instance_of_comment(self):
    ...
    self.assertEquals(self.new_comment.user,self.user_jojo)
